    ...
    async testConnection(): Promise<{ ok: boolean; message: string }> {
      try {
        ...
        if (res.status === 401 || res.status === 403) {
          return {
            ok: false,
            message: "Authentication failed (HTTP 401/403): Invalid Radarr API Key. Check Settings -> General -> Security.",
          };
        }
        ...
        if (!res.ok) {
          return {
            ok: false,
            message: `Radarr returned HTTP ${res.status}: ${res.statusText}`,
          };
        }

        const data = (await res.json()) as { appName?: string; version?: string };
        const name = data.appName || "Radarr";
        ...
        return {
          ok: false,
          message: `Could not connect to Radarr at ${baseUrl}: ${(err as Error).message}`,
        };
      }
    },
  };
